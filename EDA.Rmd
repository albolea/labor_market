...
*Wage:  
As we can see below we have some extreme cases where the earnings is US`$`356,000 that we consider as outliers. Also, we add a restriction than a worker would receive a minimum wage of US`$`5.00, so we excluded earnings that are lower than $5*40*50 = 10,000$     
```{r }
...
base %>% filter(is.na(TABLET)) %>% select(WAGP) %>% summary() #Tablet NA
...
base %>%  ggplot(aes(x=TABLET,y=WAGP,fill=TABLET)) + geom_boxplot()+
  labs(title="Earnings x Having Tablet",
       x ="Earnings", y = "Density") +
  scale_fill_discrete(name="Person has Tablet",
                      breaks=c("1", "2", "NA"),
                      labels=c("Yes", "No", "NA"))
```
...
base %>% filter(is.na(SMARTPHONE)) %>% select(WAGP) %>% summary() #smartphone NA
...
base %>%  ggplot(aes(x=SMARTPHONE,y=WAGP,fill=SMARTPHONE)) + geom_boxplot()+
  labs(title="Earnings x Having Smartphone",
       x ="Earnings", y = "Density") +
  scale_fill_discrete(name="Person has Smartphone",
                      breaks=c("1", "2", "NA"),
                      labels=c("Yes", "No", "NA"))
```
...
base %>% filter(is.na(LAPTOP)) %>% select(WAGP) %>% summary() #Laptop NA


base %>%  ggplot(aes(WAGP,color=LAPTOP)) + geom_density() + 
  labs(title="Earnings x Having Laptop",
       x ="Earnings", y = "Density") +
  scale_color_discrete(name="Person has Laptop",
                      labels=c("Yes", "No", "NA"))

base %>%  ggplot(aes(x=LAPTOP,y=WAGP,fill=LAPTOP)) + geom_boxplot()+
  labs(title="Earnings x Having Laptop",
       x ="Earnings", y = "Density") +
  scale_fill_discrete(name="Person has Laptop",
                      breaks=c("1", "2", "NA"),
                      labels=c("Yes", "No", "NA"))
```
...
base %>% filter(is.na(HISPEED)) %>% select(WAGP) %>% summary() #HIGH SPEED Internet NA
...
base %>%  ggplot(aes(x=HISPEED,y=WAGP,fill=HISPEED)) + geom_boxplot()+
  labs(title="Earnings x Having HIGH SPEED Internet",
       x ="Earnings", y = "Density") +
  scale_fill_discrete(name="Person has HIGH SPEED Internet",
                      breaks=c("1", "2", "NA"),
                      labels=c("Yes", "No", "NA"))
```