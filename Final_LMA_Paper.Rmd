...
For our analysis we have chosen to perform a series of multiple regression analyses using OLS estimators. For this analysis we will use a logarithmic transformation of the dependent variable, earnings. Even though there was no evidence of heteroscedasticity we chose to use log(earnings) because conceptually percentage changes across our independent variables makes more sense. Next, we address variable selection. In our first model the variable of most interest is included as the dummy variable Free Access. Categorical variables are included for race as Black and Hispanic with White as the reference category, this was chosen so that the coefficients of the other races can be interpreted in respect to Whites. The other set of categorical variables represent educational attainment; High School Diploma or GED, Some College, Undergraduate Degree or Higher with no High School Diploma as the reference category. These variables were selected in order to avoid omitted variable bias because educational attainment and earnings are positively correlated but not jointly determined. The dummy variable Female was included to account for gender. Lastly the variables for age and age^2 were included, age^2 was included to account for the no linearity of the relationship between earnings and age. This constructed a model of the form:
